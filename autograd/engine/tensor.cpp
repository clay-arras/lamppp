...
Tensor Tensor::matmul(const Tensor& other) const { // TODO(nlin): maybe implement as static method that takes in A and B? 
    assert(shape.size() == 2);
    assert(other.shape.size() == 2);
    assert(shape[1] == other.shape[0]);
    std::vector<float> res_data(data.size());
    Eigen::Map<Eigen::MatrixXf> res(res_data.data(), shape[0], other.shape[1]);
    res = as_matrix(shape[0], shape[1]) * other.as_matrix(other.shape[0], other.shape[1]);  
    return Tensor(res_data, shape);
}
