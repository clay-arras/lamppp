...
public:
    void register_benchmarks(const OperatorConfig<1>& config) {
        ...
        auto init_fn = [config]() -> std::array<lmp::autograd::Variable, 1> {
            return {lmp::autograd::rand(config.shapes[0], config.device, config.dtype, true)};
        };
...
public:
    ...
    void register_benchmarks(const OperatorConfig<1>& config) {
        ...
        for (size_t axis : axes) {
            auto op_fn = [this, axis](const std::array<lmp::autograd::Variable, 1>& inputs) -> lmp::autograd::Variable {
...
protected:
    virtual lmp::autograd::Variable apply_operation(const lmp::autograd::Variable& a, size_t axis) = 0;
};
...
protected:
    lmp::autograd::Variable apply_operation(const lmp::autograd::Variable& a, size_t axis) override {
