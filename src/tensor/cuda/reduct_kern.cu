...
void vecSum(const T* in, T* out, const size_t* shape, const stride_t* stride,
  ...
  ListDevicePtr<stride_t> d_stride(stride, ndims);

  size_t outSize = size / shape[axis];
  size_t threads = 256;
  size_t blocks = (outSize + threads - 1) / threads;
  vecSumKernel<<<blocks, threads>>>(const_cast<T*>(in), out, d_shape.get(),
                                    d_stride.get(), axis, outSize);
}
...
void vecMax(const T* in, T* out, const size_t* shape, const stride_t* stride,
  ...
  size_t threads = 256;
  size_t blocks = (outSize + threads - 1) / threads;
  vecMaxKernel<<<blocks, threads>>>(const_cast<T*>(in), out, d_shape.get(),
                                    d_stride.get(), axis, outSize);
}
...
void vecMin(const T* in, T* out, const size_t* shape, const stride_t* stride,
  ...
  size_t threads = 256;
  size_t blocks = (outSize + threads - 1) / threads;
  vecMinKernel<<<blocks, threads>>>(const_cast<T*>(in), out, d_shape.get(),
                                    d_stride.get(), axis, outSize);
}

// clang-format off
#define INSTANTIATE_REDUCT(r, data, elem)                              \
  template void vecSum<elem>(const elem*, elem*, const size_t*,        \
                             const stride_t*, size_t, size_t, size_t); \
  template void vecMax<elem>(const elem*, elem*, const size_t*,        \
                             const stride_t*, size_t, size_t, size_t); \
  template void vecMin<elem>(const elem*, elem*, const size_t*,        \
                             const stride_t*, size_t, size_t, size_t);
