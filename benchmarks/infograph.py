    ...
    def extract_info(name):
        ...
        if operation in ["add", "sub", "mul", "div", "pow"]:
            if "1x64x1_64x1x64" in name:
                ...
                dimensions = (
                    f"{match.group(1)}x{match.group(2)}" if match else "unknown"
                )
            ...
            axis_dims = {
                ...
                "axis0_256x32": "256x32",
                "axis1_256x32": "256x32",
                "axis0_256x512": "256x512",
                "axis1_256x512": "256x512",
            }
            dimensions = next(
                (dim for key, dim in axis_dims.items() if key in name), "unknown"
            )
        else:
...
def parse_torch_benchmarks(filepath):
    ...
    def extract_torch_info(case_name, operation):
        ...
        if operation in ["add", "sub", "mul", "div", "pow"]:
            if "[64,1,64]" in case_name and "[1,64,1]" in case_name:
                ...
                dimensions = (
                    f"{matches[0][0]}x{matches[0][1]}" if matches else "unknown"
                )
        elif operation in ["sum", "min", "max", "prod"]:
            reduction_dims = {
                "R64_V32": "64x32",
            ...
            dimensions = next(
                (dim for key, dim in reduction_dims.items() if key in case_name),
                "unknown",
            )
        else:
    ...
    parsed_data = []
    for _, row in df.iterrows():
        device, dimensions = extract_torch_info(
            row["Case Name"], row["Benchamrking Module Name"]
        )
        parsed_data.append(
            {
                "framework": "PyTorch",
...
def load_and_process_data():
    ...
    combined_df["operation_category"] = combined_df["operation"].apply(
...
def plot_performance_by_size(data):
    ...
    for i, device in enumerate(devices):
        for j, pass_type in enumerate(passes):
            ...
            if not subset.empty:
                perf_by_size = (
                    subset.groupby(["framework", "dimensions"])["execution_time_ms"]
                    .mean()
                    .reset_index()
                )
                pivot_data = perf_by_size.pivot(
                    index="dimensions", columns="framework", values="execution_time_ms"
                )
...
def plot_speedup_analysis(data):
    ...
    speedup_data = (
        data.groupby(["operation", "device", "forward_backward", "dimensions"])
    ...
    avg_speedup = (
        speedup_data.groupby(["device", "forward_backward"])["speedup"]
        .mean()
        .reset_index()
    )
    pivot_speedup = avg_speedup.pivot(
...
if not speedup_data.empty:
    ...
    if not cuda_speedup.empty:
        ...
        print(
            f"This means LMP is {overall_avg:.2f}x faster than PyTorch on average on CUDA"
        )
        ...
        for pass_type in cuda_speedup["forward_backward"].unique():
            pass_speedup = cuda_speedup[cuda_speedup["forward_backward"] == pass_type][
                "speedup"
            ].mean()
        ...
        print("-" * 50)
        op_speedup = (
            cuda_speedup.groupby("operation")["speedup"]
            .mean()
        ...
        if len(op_speedup) > 0:
            ...
            print(
                f"Worst performing operation: {worst_op} ({op_speedup[worst_op]:.2f}x)"
            )
    else:
...
def plot_operation_performance(data):
    ...
    operations_with_both = (
        data.groupby("operation")
        .filter(lambda x: len(x["framework"].unique()) > 1)["operation"]
        .unique()
    )
    ...
    for category in ["Binary", "Unary", "Reduction", "Other"]:
        if op_categories[category]:
            category_boundaries[category] = (
                current_pos,
                current_pos + len(op_categories[category]) - 1,
            )
    ...
    fig.suptitle(
        "Performance Comparison by Operation (Grouped by Type)",
        fontsize=16,
        fontweight="bold",
    )
    ...
    for idx, device in enumerate(["CPU", "CUDA"]):
        ...
        if not device_data.empty:
            perf_data = (
                device_data.groupby(["framework", "operation"])["execution_time_ms"]
                .mean()
                .reset_index()
            )
            pivot_data = perf_data.pivot(
                index="operation", columns="framework", values="execution_time_ms"
            )
            pivot_data = pivot_data.fillna(0)
            pivot_data = pivot_data.reindex(
                [op for op in ordered_operations if op in pivot_data.index]
            )

            pivot_data.plot(kind="bar", ax=axes[idx], color=COLORS[:2])
            style_axis(
                axes[idx],
                f"{device} Performance by Operation",
                "Operation",
                "Average Execution Time (ms)",
            )
            ...
            for category in ["Binary", "Unary", "Reduction", "Other"]:
                if category in category_boundaries:
                    if x_pos > 0:
                        axes[idx].axvline(
                            x=x_pos - 0.5, color="red", linestyle="--", alpha=0.7
                        )
...
def print_summary_statistics(data, speedup_data):
    ...
    summary_stats = (
        data.groupby(["framework", "device"])
        .agg({"execution_time_ms": ["mean", "median", "std", "min", "max", "count"]})
        .round(4)
    )
    print(summary_stats)
    ...
    if speedup_data is not None and not speedup_data.empty:
        ...
        speedup_stats = (
            speedup_data.groupby(["device"])["speedup"]
        ...
        top_speedups = speedup_data.nlargest(5, "speedup")[
            ["operation", "device", "forward_backward", "speedup"]
        ]
        print(top_speedups.to_string(index=False))

        print("\nTop 5 scenarios where PyTorch is fastest compared to LMP:")
        worst_speedups = speedup_data.nsmallest(5, "speedup")[
            ["operation", "device", "forward_backward", "speedup"]
        ]
