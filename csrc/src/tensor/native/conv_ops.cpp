...
Tensor conv1d(const Tensor& input, const Tensor& kernel, size_t stride,
  ...
  return detail::UnsafeTensorAccessor::fromImpl(std::make_shared<TensorImpl>(
      conv1d_stub()(input.device(), *detail::UnsafeTensorAccessor::getImpl(input),
...
Tensor conv2d(const Tensor& input, const Tensor& kernel, size_t stride,
  ...
  LMP_CHECK(input.device() == kernel.device())
      << "Tensors are on different devices";
  return detail::UnsafeTensorAccessor::fromImpl(std::make_shared<TensorImpl>(
      conv2d_stub()(input.device(), *detail::UnsafeTensorAccessor::getImpl(input),
                  *detail::UnsafeTensorAccessor::getImpl(kernel), stride,
                  padding, dilation)));
}
...
Tensor conv3d(const Tensor& input, const Tensor& kernel, size_t stride,
  ...
  LMP_CHECK(input.device() == kernel.device())
      << "Tensors are on different devices";
  return detail::UnsafeTensorAccessor::fromImpl(std::make_shared<TensorImpl>(
      conv3d_stub()(input.device(), *detail::UnsafeTensorAccessor::getImpl(input),
                  *detail::UnsafeTensorAccessor::getImpl(kernel), stride,
                  padding, dilation)));
}
