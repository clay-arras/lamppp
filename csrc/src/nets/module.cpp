...
std::vector<std::reference_wrapper<Parameter>> ModuleImpl::parameters() {
  ...
  for (auto& [k, module] : modules_) {
    std::vector<std::reference_wrapper<Parameter>> child_params =
        module->parameters();  // recursive call
...
// NOLINTNEXTLINE(misc-no-recursion)
std::multimap<std::string, std::reference_wrapper<Parameter>>
ModuleImpl::named_parameters() {  // NOLINT(misc-no-recursion)
  std::multimap<std::string, std::reference_wrapper<Parameter>> all_params;
  for (auto& [k, v] : params_) {
    all_params.insert({k, v});
  }
  for (auto& [k, module] : modules_) {
    std::multimap<std::string, std::reference_wrapper<Parameter>> child_params =
        module->named_parameters();  // recursive call
...
void ModuleImpl::register_module(const std::string& name,
