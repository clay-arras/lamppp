...
template <typename U, typename Alloc> struct IsVector<std::vector<U, Alloc>> : std::true_type {};
struct TensorHelper {
    ...
    void unroll(const std::vector<T>& tensor, int depth = 0) {
        ...
        if constexpr (IsVector<T>::value) {
            ...
            data.insert(data.end(), tensor.begin(), tensor.end()); // TODO(nlin): can use memcpy, it's faster
        }
    }
};
