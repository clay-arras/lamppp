#include "lamp3/tensor/cpu/conv.hpp"
...
template <typename U, typename V, typename OutType>
void cpuConv2dKernel(const U* input, const V* kernel, OutType* output,
                     size_t stride, size_t padding, size_t dilation,
                     const size_t* input_shape, const size_t* kernel_shape,
                     const size_t* output_shape, size_t i, size_t j) {
  OutType sum =
      0;  // TODO(nx2372): optimization, use shared memory instead of global reads
  ssize_t start_i = (i * stride) - padding;
...
template <typename U, typename V, typename OutType>
void cpuConv3dKernel(const U* input, const V* kernel, OutType* output,
                     size_t stride, size_t padding, size_t dilation,
                     const size_t* input_shape, const size_t* kernel_shape,
                     const size_t* output_shape, size_t i, size_t j, size_t k) {
  OutType sum = 0;
  ssize_t start_i = (i * stride) - padding;
  ssize_t start_j = (j * stride) - padding;
...
template <typename U, typename V, typename OutType>
void cpuConv1d(const U* input, const V* kernel, OutType* output, size_t stride,
               size_t padding, size_t dilation, const size_t* input_shape,
               const size_t* kernel_shape, const size_t* output_shape) {
#pragma omp parallel for schedule(static)
  for (size_t i = 0; i < output_shape[0]; i++)
    cpuConv1dKernel<U, V, OutType>(input, kernel, output, stride, padding,
                                   dilation, input_shape, kernel_shape,
                                   output_shape, i);
}

template <typename U, typename V, typename OutType>
void cpuConv2d(const U* input, const V* kernel, OutType* output, size_t stride,
               size_t padding, size_t dilation, const size_t* input_shape,
               const size_t* kernel_shape, const size_t* output_shape) {
#pragma omp parallel for collapse(2) schedule(static)
  for (size_t i = 0; i < output_shape[0]; i++)
    for (size_t j = 0; j < output_shape[1]; j++)
      cpuConv2dKernel<U, V, OutType>(input, kernel, output, stride, padding,
                                     dilation, input_shape, kernel_shape,
                                     output_shape, i, j);
}

template <typename U, typename V, typename OutType>
void cpuConv3d(const U* input, const V* kernel, OutType* output, size_t stride,
               size_t padding, size_t dilation, const size_t* input_shape,
               const size_t* kernel_shape, const size_t* output_shape) {
  ...
  for (size_t i = 0; i < output_shape[0]; i++)
    for (size_t j = 0; j < output_shape[1]; j++)
      for (size_t k = 0; k < output_shape[2]; k++)
        cpuConv3dKernel<U, V, OutType>(input, kernel, output, stride, padding,
                                       dilation, input_shape, kernel_shape,
                                       output_shape, i, j, k);
}
